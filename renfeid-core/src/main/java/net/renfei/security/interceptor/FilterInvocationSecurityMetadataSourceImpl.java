...
public class FilterInvocationSecurityMetadataSourceImpl implements FilterInvocationSecurityMetadataSource {
    ...
    private final List<SysApiList> sysApiList;

    public FilterInvocationSecurityMetadataSourceImpl(SysService sysService) {
        this.sysService = sysService;
        this.antPathMatcher = new AntPathMatcher();
        // 从数据库加载权限配置
        this.sysApiList = sysService.getSysApiAllList();
    }
    ...
    public Collection<ConfigAttribute> getAttributes(Object object) throws IllegalArgumentException {
        ...
        // /error/ 开头的是错误页面，直接放行
        if (!filterInvocation.getRequestUrl().startsWith("/error/")) {
            ...
            if (requestUrl.startsWith("/_/api/")) {
                ...
                for (SysApiList sysApi : sysApiList
                ) {
                    // 遍历系统所有的资源进行匹配
