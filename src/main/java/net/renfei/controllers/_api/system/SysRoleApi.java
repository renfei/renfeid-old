...
public interface SysRoleApi {
    ...
    APIResult<ListData<SysRoleVO>> getSysRoleList(@RequestParam(value = "pages", required = false) String pages,
                                                  @RequestParam(value = "rows", required = false) String rows);

    @PostMapping("sys/role")
    @Operation(summary = "添加系统角色", tags = {"系统接口列表"}, description = "只有超管和安全管理员可以添加角色")
    APIResult addSysRole(@RequestBody SysRoleVO sysRole);

    @PutMapping("sys/role/{id}")
    @Operation(summary = "修改系统角色", tags = {"系统接口列表"}, description = "只有超管和安全管理员可以修改系统角色")
    APIResult updateSysRole(@PathVariable("id") Long id, @RequestBody SysRoleVO sysRole);
